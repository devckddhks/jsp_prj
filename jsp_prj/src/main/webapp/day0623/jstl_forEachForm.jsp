...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			<!-- form 태그에 action 속성을 정의하지 않으면 submit이 되었을 때 자신 페이지를 재요청하게된다. -->
			<form method="post">
				<label>이름</label><input type="text" name="name" value="test"><br>
				<label>관심언어</label>
				<%
				...
				<c:forEach var="subject" items="${ subjectData }" varStatus="i">
					<c:out value="${ i.index }"/>.
					<c:if test="${ i.first }"><strong></c:if>
					<input type="checkbox" name="lang" value="${ subjectData }">
					<c:if test="${ i.first }"></strong></c:if>
					<c:out value="${ subject }"/>
				</c:forEach>
			...
			<div>
				<c:if test="${ not empty param.name }">
					<!-- wep parameter에 name값이 존재하면 선택된 checkbox의 값을 출력 -->
					<span><c:out value="${ param.name }"/></span><span>님이 선택하신 언어는</span><br>
					<c:if test="${ empty varamValue.legn }">
						<ul>
							<c:forEach var="lang" items="${ paramValues.lang }" varStatus="i">
								<li>
									<c:out value="${ i.count }.${ lang }"></c:out>
								</li>
							</c:forEach>
						</ul>
					</c:if>		
						...
						<c:set var="totalAge" value="${ uDTO2.age + totalAge}" />
							...
							<c:if test="${ i.last }">
								<tr>
									...
									<td><c:out value="${ totalAge }세"></c:out></td>
								</tr>
							</c:if>
						</c:forEach>
					</tbody>
