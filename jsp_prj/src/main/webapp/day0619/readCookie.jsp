					...
					if(cookies != null){
						...
						for(int i = 0; i < cookies.length; i++) {
							...
							if(!"JSESSIONID".equals(tempCookie.getName())) {
								...
								if("chk".equals(tempCookie.getName())) {
									cookieChk = tempCookie.getValue();
								}
							}
						}
					}
...
<%-- 						쿠키 값 : 이름 <span><%= cookieName %></span>, 
					...
					%>
						<form action="addCookie.jsp" method="get" id="frm">
							<label>이름</label> <input type="text" name="name" id="name" value="<%= cookieName %>"/> <br> 
							<label>나이</label> <input type="text" name="age" id="age" value="<%= cookieAge %>"> <br>
							<!-- 이름과 나이 저장을 쿠키 상태에 따라 checked설정을 해보세요. -->
							<input type="checkbox" name="useCookie" <%= "on".equals(cookieChk) ? "checked='checked'" : "" %>> 이름과 나이 저장 
							
							<input type="button" value="입력" class="btn btn-outline-success btn-sm" id="btn">
						</form>
					<%
					// }
					%>
